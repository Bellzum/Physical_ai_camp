...
bk_err_t bk_agora_rtc_start(agora_rtc_option_t *option)
{
    ...
    // Improve voice quality for chatroom-style calls.
    // We cannot set Agora audio profile enums directly in this wrapper,
    // so we use the available SDK audio processing features:
    // - keep jitter buffer enabled for smooth packet arrival
    // - enable audio process, AEC and noise suppression for clearer voice
    // This matches the spirit of: CHATROOM + MUSIC_STANDARD for expressive voice.
    channel_options.audio_process_opt.enable_audio_process = true;
    channel_options.audio_process_opt.enable_aec = true;
    channel_options.audio_process_opt.enable_downlink_aec = false;
    channel_options.audio_process_opt.enable_ns = true;
    channel_options.audio_process_opt.ref_data_from_sdk = false;
    channel_options.audio_process_opt.enable_dump_data = false;
    // open jitter buffer
